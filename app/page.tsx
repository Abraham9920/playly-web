      ...
      if (res.ok) {
        ...
        if (data.length > 0) {
          const mapped = data.map((g: any) => ({
            id: g.id,
            sport: g.sport
              ? g.sport.charAt(0) + g.sport.slice(1).toLowerCase()
              : "Sport",
            title: g.title,
            venue:
              g.venue && typeof g.venue === "object"
                ? g.venue.name || "NYC"
                : g.venue || "NYC",
            address:
              g.venue && typeof g.venue === "object"
                ? g.venue.address || "New York"
                : "New York",
            ...
            players: g.joinedPlayers || (g.bookings ? g.bookings.length : 0),
            maxPlayers: g.maxPlayers,
            level:
              g.skillLevel === "ALL_LEVELS"
                ? "All Levels"
                : g.skillLevel
